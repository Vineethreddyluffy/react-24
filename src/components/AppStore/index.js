...
import GetItem from '../TabItem'

import GetSelected from '../AppItem'
...
class AppStore extends Component {
  ...
  onClickItems = () => {
    ...
    const newList = appsList.filter(each => each.category === activeTab)
  ...
  render() {
    const {activeTab} = this.state
    const searchResults = this.onClickItems()
    return (
      <div className="cont">
        ...
        <ul className="card-cont">
          {searchResults.map(each => (
            <GetSelected key={each.appId} items={each} />
          ))}
        </ul>
      </div>
    )
